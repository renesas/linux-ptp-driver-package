...
static int rsmu_get_tdc_apll_freq(struct rsmu_cdev *rsmu)
{
	...
	TDC_APLL(rsmu) = (u32)div_u64(tdc_ref_freq * (u64)tdc_fb_div_int, (u32)1 << tdc_ref_div);
...
static s64 rsmu_get_fod_freq(struct rsmu_cdev *rsmu, u8 fod_n)
{
	...
	div_int = (u16)FIELD_GET(FOD_DIV_INTEGER, div_cnfg);
...
static s64 rsmu_get_time_clk_freq(struct rsmu_cdev *rsmu)
{
	...
	if (err)
		return err;
	time_clk_div = (u8)((val & TIME_CLOCK_COUNT_MASK) + 1);
...
static int rsmu_get_time_ref_freq(struct rsmu_cdev *rsmu)
{
	...
	if (time_clk_freq < 0)
		return (int)time_clk_freq;

	err = regmap_bulk_read(rsmu->regmap, TIME_CLOCK_MEAS_DIV_CNFG, buf, sizeof(buf));
	if (err)
		return err;
	time_ref_div = (u8)(FIELD_GET(TIME_REF_DIV_MASK, get_unaligned_le32(buf)) + 1);

	err = regmap_bulk_read(rsmu->regmap, TIME_CLOCK_COUNT, buf, 1);
	if (err)
		return err;
	time_clk_div = (u8)((buf[0] & TIME_CLOCK_COUNT_MASK) + 1);
	TIME_REF(rsmu) = (u32)div_u64((u64)time_clk_freq * (u64)time_clk_div, time_ref_div);
...
static int load_firmware(struct rsmu_cdev *rsmu, char fwname[FW_NAME_LEN_MAX])
{
	...
	for (len = fw->size; len > 0; len -= sizeof(*rec)) {
		val = rec->value;
		addr = (u16)(rec->hiaddr << 8 | rec->loaddr);
				   ...
				   u8 *state)
{
	...
	if (devid == VFC3A)
		...
		reg_addr += (u32)(dpll * 0x100);

	err = regmap_bulk_read(rsmu->regmap, reg_addr, &reg, sizeof(reg));
	if (err)
		return err;

	reg = (u8)((reg & DPLL_STATE_STS_MASK) >> DPLL_STATE_STS_SHIFT);
				    ...
				    s8 *clock_index)
{
	...
	*clock_index = -1;
	...
	if ((dpll_state_sts == DPLL_STATE_LOCKED) || (dpll_state_sts == DPLL_STATE_ACQUIRE) ||
		...
		ref_index = (u8)((dpll_sts_reg & DPLL_REF_SEL_STS_MASK) >> DPLL_REF_SEL_STS_SHIFT);
					 ...
					 struct rsmu_priority_entry *priority_entry)
{
	...
	for (priority_index = 0; priority_index < number_entries; priority_index++) {
		...
		switch (ref_index) {
		case 0:
			reg = (u16)((reg & (~DPLL_REF0_PRIORITY_ENABLE_AND_SET_MASK)) |
			       (priority << DPLL_REF0_PRIORITY_SHIFT));
			break;
		case 1:
			reg = (u16)((reg & (~DPLL_REF1_PRIORITY_ENABLE_AND_SET_MASK)) |
			       (priority << DPLL_REF1_PRIORITY_SHIFT));
			break;
		case 2:
			reg = (u16)((reg & (~DPLL_REF2_PRIORITY_ENABLE_AND_SET_MASK)) |
			       (priority << DPLL_REF2_PRIORITY_SHIFT));
			break;
		case 3:
			reg = (u16)((reg & (~DPLL_REF3_PRIORITY_ENABLE_AND_SET_MASK)) |
					...
					struct rsmu_reference_monitor_status_alarms *alarms)
{
	...
	alarms->frequency_offset_limit = (u8)((freq_reg >> FREQ_FAIL_STS_SHIFT) & 1);
