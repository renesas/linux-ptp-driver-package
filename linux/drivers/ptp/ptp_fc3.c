...
static int idtfc3_set_lpf_mode(struct idtfc3 *idtfc3, u8 mode)
{
	...
	if (mode == LPF_DISABLED)
		return idtfc3_enable_lpf(idtfc3, false);

	return idtfc3_enable_lpf(idtfc3, true);
}
...
static int idtfc3_lpf_bw(struct idtfc3 *idtfc3, u8 shift, u8 mult)
{
	...
	/* Disable filter */
	err = regmap_bulk_write(idtfc3->regmap, LPF_DIS_CTRL, &val, sizeof(val));
	if (err)
		return err;

	val = FIELD_PREP(LPF_BW_SHIFT, shift) | FIELD_PREP(LPF_BW_MULT, mult);
	err = regmap_bulk_write(idtfc3->regmap, LPF_BW_CNFG, &val, sizeof(val));
	if (err)
		return err;

	/* Enable filter */
	val = 0;
	return regmap_bulk_write(idtfc3->regmap, LPF_DIS_CTRL, &val, sizeof(val));
}
...
static int idtfc3_hw_calibrate(struct idtfc3 *idtfc3)
{
	...
	/*
	 * Toggle SOFT_RESET:
	 * (1) set soft_reset to 0
	 * (2) set soft_reset to 1
	 */
	val = 0;
...
static int idtfc3_configure_hw(struct idtfc3 *idtfc3)
{
	...
	(void)wait_for_sys_apll_dpll_lock(idtfc3);

	err = idtfc3_enable_lpf(idtfc3, false);
	if (err)
		return err;
